...
vec3 AddLight()
{
	...
	vec3 normalMapValue = oVertexNormal;
	...
	vec3 diffuseFactor = vec3(diffuse);// * texture(entityTexture,oTextureCoord).rgb;
	...
	vec3 ambientFactor = vec3(ambientStrength) ;//* texture(entityTexture,oTextureCoord).rgb;
